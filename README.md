...
只针对网易云音乐客户端下载得到的 `.ncm` 文件，不支持 `.qmc/.mflac` 等其他加密格式。程序在本地还原内嵌 FLAC/MP3 音频，并尽力写入歌曲名、歌手、专辑和封面，不上传文件。
...
```powershell
python -m py_compile main.py converter.py downloader.py ncm_decrypt.py premium.py updater.py i18n.py
python -m unittest discover tests -v
```

覆盖 URL 校验与平台识别、yt-dlp 选项、FFmpeg/ffprobe 定位、24-bit 音频转码、原子输出、NCM 封面段解析与 C 层 XOR 解密、元数据写入、Premium 参数校验、更新模块的版本比较/限流兜底，以及界面文案的翻译覆盖率。
